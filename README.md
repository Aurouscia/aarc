...
4. 尝试启动并进入网站
5. 特别注意：不要让 IIS 的 Idle Time-out（默认20分钟）小于 AARC 的心跳机制间隔（默认3分钟）确保有人在编辑时，进程不被回收
