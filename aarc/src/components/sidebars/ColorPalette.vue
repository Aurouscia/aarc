...
import { useExternalColorSetsStore } from '@/app/localConfig/externalColorSets';
import { useSaveStore } from '@/models/stores/saveStore';
import { storeToRefs } from 'pinia';
import { removeAllByPred } from '@/utils/lang/indicesInArray';
import { keepOrderSort } from '@/utils/lang/keepOrderSort';
import { loadAndMergeColorSets, initColorSetMeta, parseColorSetData, type ColorSetExtended } from './colorPalette';
...
colorSetsProm.then(x => {
    const builtIn = x.default as ColorSet[]
    const merged = loadAndMergeColorSets(
        builtIn,
        externalColorSetsStore.groups,
        saveStore.save?.dataSources ?? []
    )
    cs.value = merged
    initColorSetMeta(cs.value)
    ensureSetsOrdered()
})
...
function toggleSetShowing(colorSet:ColorSetExtended){
    if(colorSet.showing){
        ...
        if(!colorSet.items){
            const items = parseColorSetData(colorSet.data)
            for (const item of items) {
                item.colorInv = colorProcStore.colorProcInvBinary.convertNoCache(item.color)
            }
            colorSet.items = items
        }
    }
}
...
function isFavorite(cs:ColorSetExtended|string){
    const name = typeof cs =='string' ? cs : cs.name.trim()
    return fav.value.has(name)
}
function toggleFavorite(csItem:ColorSetExtended){
    const name = csItem.name.trim()
    if(isFavorite(name)){
        removeAllByPred<string>(favoriteNames.value, x => x == name)
        csItem.showing = false
    }
    else
