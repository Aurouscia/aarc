...
<template>
    <SideBar ref="sidebar" @extend="init">
        ...
        <div class="lineConfig">
            <table v-if="!line.parent" class="fullWidth">
                <tbody>
                    <tr>
                        <td colspan="2">
                            {{ line.name || '未命名线路' }}
                    ...
                    <tr>
                        <td>分组</td>
                        <td>
                            <select v-model="line.group"
                                @change="saveStore.ensureLinesOrdered(); envStore.lineInfoChanged(line)">
                    ...
                    <tr>
                        ...
                        <td class="viewableRange">
                            <input type="range" v-model="lineWidthBinded" :min="lineWidthRange.min"
                                :max="lineWidthRange.max" :step="lineWidthRange.step" value="1"
                                @change="lineWidthChanged" />
                            <input v-if="line.type === LineType.terrain" type="number" v-model="lineWidthBinded"
                                @blur="lineWidthChanged" />
                    ...
                    <tr>
                        ...
                        <td v-if="line.type === LineType.common">
                            <div v-if="line.whiteStroke || line.whiteStroke == undefined">
                                <input type="checkbox" v-model="line.whiteStroke"
                                    @change="envStore.lineWhiteStrokeChanged(line)" checked />
                            </div>
                            <div v-else>
                                <input type="checkbox" v-model="line.whiteStroke"
                                    @change="envStore.lineWhiteStrokeChanged(line)" />
                            </div>
                        </td>
                    </tr>
                    ...
                    <tr v-if="line.type === LineType.common">
                        <td>站名</td>
                        <td class="viewableRange">
                            <input type="range" v-model="lineStaNameSizeBinded" :min="0" :max="2" :step="0.25"
                                @change="lineStaNameSizeChanged" />
                            <input type="number" v-model="lineStaNameSizeBinded" :min="0" :max="2" :step="0.05"
                                @change="lineStaNameSizeChanged" />
                    ...
                    <tr v-if="line.type === LineType.common">
                        <td>车站</td>
                        <td class="viewableRange">
                            <input type="range" v-model="lineStaSizeBinded" :min="0" :max="2" :step="0.25"
                                @change="lineStaSizeChanged" />
                            <input type="number" v-model="lineStaSizeBinded" :min="0" :max="2" :step="0.05"
                                @change="lineStaSizeChanged" />
                            <div class="smallNote">(设为0使用"设置-线宽对应<br />车站尺寸"中的全局设置)</div>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td>层级</td>
                        <td class="viewableRange">
                            <input type="range" v-model="lineZIndexBinded" :min="-9" :max="9" :step="1"
                    ...
                    <tr v-if="!line.parent">
                        ...
                        <td v-if="!haveChildren">
                            <select v-model="selectedForParentAssignment" style="max-width: 160px;">
                                ...
                                <option v-for="l in selectableParent" :value="l.id">{{ l.name ?? '未命名线路' }}</option>
                            </select><br />
